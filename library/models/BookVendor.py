...
class BookVendorManager(BaseModelManager):
    def filter_search(self, qs, search):
        ...
        for col in self.model.order_columns:
            ...
            if col == 'vendor_name':
                q |= Q(vendor_name__icontains=search)
...
class BookVendor(BaseModel):
    class Meta:
        db_table = '"library_book_vendor"'
    ...
    vendor_phone = models.BigIntegerField(null=True)
    vendor_email = models.CharField(max_length=255, null=True)
    status = models.SmallIntegerField(default=1)

    columns = ['id','vendor_name','vendor_address','vendor_phone','vendor_email','status']
    order_columns = ['id','vendor_name','vendor_address','vendor_phone','vendor_email','status','']
    
    objects = models.Manager() # The default manager.
    filter_objects = BookVendorManager()
    def __str__(self):
