...
class CupBoardManager(BaseModelManager):
    def filter_search(self, qs, search):
        ...
        for col in self.model.order_columns:
            ...
            if col == 'cupboard_name':
                q |= Q(cupboard_name__icontains=search)
            
            if col == 'available_class.classname':
                q |= Q(available_class__class_name__icontains=search)
...
class CupBoard(BaseModel):
    class Meta:
        db_table = '"library_book_cupboard"'
    # firm_id = models.IntegerField(null=True)
    # dept_id = models.IntegerField(null=True)
    # org_id = models.IntegerField(null=True)
    cupboard_name = models.CharField(max_length=255, null=True)
    # cupboard_shelf_name = models.CharField(max_length=255, null=True)
    cupboard_details = models.CharField(max_length=255, blank=True, null=True)
    status = models.SmallIntegerField(default=1)

    columns = ['id','cupboard_name','status']
    order_columns = ['id','cupboard_name','status','']
   
    objects = models.Manager() # The default manager.
    filter_objects = CupBoardManager()

    def __str__(self):
        return "%s" % (self.cupboard_name)
