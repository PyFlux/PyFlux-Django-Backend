...
class ReligionManager(BaseModelManager):
    def filter_search(self, qs, search):
        ...
        for col in self.model.order_columns:
            ...
            if col == 'religion_name':
                q |= Q(religion_name__icontains=search)
...
class Religion(BaseModel):
    class Meta:
        db_table = '"system_config_religion"'
    # firm_id = models.IntegerField(null=True)
    # dept_id = models.IntegerField(null=True)
    # org_id = models.IntegerField(null=True)
    religion_name = models.CharField(max_length=255, null=True)
    status = models.SmallIntegerField(default=1)

    columns = ['id','religion_name','status']
    order_columns = ['id','religion_name','status','']
    
    
    objects = models.Manager() # The default manager.
    filter_objects = ReligionManager()

    def __str__(self):
        return "%s" % (self.religion_name)